...
<script type="text/javascript"> 
  var drugname=[]; var drugid=[];var drugstrength=[]; var drugtype=[]; var opdstk=[]; var lastpricepur=[]; 
  var tablereport;  var myvar; var drugcount; var drugs900=[]; var gstartdt; var done900sets=false;
...
function addheader()  {  
  ...
  var descreport='<thead id="idviewthead"><tr><th class="text-center">Drug name</th><th class="text-center">Strength</th>'+
...
// below variables are retunbed from backend and contains all the drugs         
       drugname=fields.drugname; drugid=fields.drugid; drugstrength=fields.drugstrength; drugtype=fields.drugtype;opdstk=fields.opdstk; 
       lastpricepur=fields.lastpricepur;
...
//  $.ajaxSetup({async: false});
  ...
  if ( (vstartdt==datestr)  && ( vdrug!="Z") ) { // if for current date and sepcific drug then get  stock from drugmaster
      ...
      $('#idproc').removeClass('hidden')  
        ...
        tablereport.row.add( [vdrugname,vstrength,vdrugtype,vstk,vaunitprice,vcost])    
               .draw(false); 
  ...
  if ( (vstartdt==datestr)  && ( vdrug=="Z") ) {// if for all drugs then get all stock  from drug master
      ...
      for(var i = 0; i < (drugname.length); i++) {
				 var vcost=(opdstk[i]*lastpricepur[i]).toFixed(2);  
        tablereport.row.add( [drugname[i], drugstrength[i], drugtype[i], opdstk[i],lastpricepur[i],vcost]) 
				  $('#idproc').removeClass('hidden') 
	         }
  ...
  if ( vdrug!="Z")  { //  if sepcific drug and specific date
      ...
      if (!fields.succfail) { 
        ...
        var vstock=fields.clsgstk; var vprice=fields.aunitprice;
                          //addedby shoba   
        vdrugname= drugval[1].substr(4); vstrength= drugval[2].substr(8); vdrugtype= drugval[3].substr(4); 
					 var vcost=(vstock*vprice).toFixed(2); 
             tablereport.row.add( [vdrugname,vstrength,vdrugtype,vstock,vprice,vcost])    
               .draw(false); 
             tablereport.draw();  
...
//    var arraylength=parseInt(drugid.length/400); var countlast=(drugid.length)%400;
//    if (countlast !=0){arraylength++};

 //     drugcount=0;drugs900 = drugid.slice(drugcount, drugcount+900);
      gstartdt=vstartdt;
//      done900sets=false;
      getnext();
 }
...
function getnext(){ 
 //04112025 var vpost=$.post("ph_opdgetstockdata.php", {palldrugs:"y", startdt:gstartdt, pdrugsall:drugs900})
          $.post("ph_opdgetstockdata.php", {palldrugs:"y", startdt:gstartdt})
           ...
           .done(function(result, status){  
            ...
            if (!fields.succfail) { 
               msg='Program terminating with error '+ fields.msg   
      // 04112025        alert(msg);done900sets=true;
              alert(msg);
               return;} 
            var aclsgstk=fields.aclsgstk;var aunitprice=fields.aunitprice; var adrugname=fields.adrugname;var adrugid=fields.adrugid;
            var adrugstrength=fields.adrugstrength;var adrugtype=fields.adrugtype;
            var vcost=0;
    
                for(var i = 0; i < (adrugid.length); i++) {
      //         rptaunitprice=0.00;rptaclsgstk=0;
      //         j = adrugname.indexOf(drugname[i])        
       //     if (j != -1) { 
        //       rptaunitprice=(aunitprice[j]*1).toFixed(2);rptaclsgstk=aclsgstk[j];}  
             
             var vcost=(aunitprice[i]*aclsgstk[i]).toFixed(2)
                  
                                                          //addedby shoba   
           tablereport.row.add( [adrugname[i],adrugstrength[i],adrugtype[i],aclsgstk[i],aunitprice[i],vcost])    
               .draw(false);   }
      ...
      if (pos != -1) { //search successful
         ...
         var  opt = drugname[i];  
           ...
           el.value = drugid[i].toString()+'--name'+drugname[i]+'--strength'+drugstrength[i]+'--type'+
						 drugtype[i]+'--stock'+opdstk[i]+'--price'+lastpricepur[i];  
  ...
  <div  class="container-fluid">
   ...
   <div class="row form-group">
        <div class="col-sm-offset-2 col-sm-2">
          <label class="radio-inline text-success font16 "><strong>OPD STOCK POSITION</strong></label></div>
