...
function addheader()  {  
 ...
 /* var descreport='<thead id="idviewthead"><tr><th class="text-center">Drug id</th><th class="text-center">Drug name</th><th class="text-center">Trans dt</th>'+
  '<th class="text-center">Transaction</th><th class="text-center">opn Stk</th><th class="text-center">Trans Qty</th><th class="text-center">Cls stk</th></tr></thead>';*/
	var descreport='<thead id="idviewthead"><tr><th class="text-center">Drug name</th><th class="text-center">Trans dt</th>'+
  '<th class="text-center">Transaction</th><th class="text-center">opn Stk</th><th class="text-center">Trans Qty</th><th class="text-center">Cls stk</th><th class="text-center">Issue Authority</th></tr></thead>';
...
function getopdaudittrail(vdrug,vstartdt,venddt) {
  ...
  $.post("ph_getopdaudittrail.php", {  pdrug:vdrugid,
...
// below variables are retunbed from backend and contains all the drugs         
      ...
      dtofissue=fields.dtofissue; vhospitalname=fields.hospitalname; 
				 ...
				 transtype=fields.transtype;time=fields.time;issueauthority=fields.issueauthority
      var vpatientname=fields.patientname; 
				 var vopdstk=fields.opdstk
...
// now add to the report table  
           ...
           tablereport.row.add( [
    vdrugname[i],trdtddmmyyyy,vhospitalname[i],
   vopstk[i],vqtyissued[i],vopdstk[i],issueauthority[i]]);
//               .draw(false); 
...
$(document).ready(function(){ 
  ...
  $('#idviewdrug').hide()
/*  var  x= "http://www.collectedworksofsriaurobindo.com/pharmacy/ph_opdpharma.php";

  
 var y="http://www.collectedworksofsriaurobindo.com/pharmacy/auropharmamenu.php";

  if (document.referrer == x) {
   $('#idrtmn').addClass('hidden');
   $('#idexit').removeClass('hidden');
  }
  else
    if (document.referrer == y)
  {
      $('#idexit').addClass('hidden');
    $('#idrtmn').removeClass('hidden');
  }*/
  var str= document.referrer
var x = str.indexOf("auropharmamenu.php");
...
$(document).on('click','#idpurbtnsrch',  function(){
 ...
 var vdrug= $('#idtextviewdrug').val();
   ...
   getopdaudittrail(vdrug,vstartdt,venddt) 
  ...
  <div  class="container-fluid">
   ...
   <div class="row form-group">
        <div class="col-sm-offset-1 col-sm-3">
          <label class="radio-inline text-success font16 "><strong>OPD PHARMACY STOCK AUDIT TRAIL</strong></label></div>
          
     <div class="col-sm-6">
    <a id="idrtmn" href="auropharmamenu.php" > <h4 class="pull-right"><strong>RETURN TO MAIN MENU</strong></h4></a>
    </div>
    <div class="col-sm-2">
    <a id="idexit" href="ph_opdpharma.php" > <h4 class="pull-right"><strong>OPD PHARMACY</strong></h4></a>
     </div>
     </div>
