<?php 
/* this is a comment
Synopsis: The program is the main program that calls the different programs, extracts data and populates the fields through javascript.
Called for first time it calls jqdocs which returns all the doctors and the phone and spec for first doctor. Jqmain then populates
the dropdown with all doctors and puts the first doctor name and spec in the fields. It enables the add modify button and disables 
everything else.
Everytime there is a change in the dropdown selection it takes the selected one and gets the phone and spec from jqdocs
which uses a simple select SQL. Clicking on Add/Modify buttons shows the input boxes for entry/modification. We use JSvalidator a plugin 
to make sure input is not empty. We also use jquery to immediately show the Confirm message as soon as all fields become valid, A "Y"
on confirm screen calls jqdocs to insert/update record. 
...
<div class="container">
  ...
  <h3 class="text-center" style="color: blue"><strong>SRI AUROBINDO ASHRAM PATIENT RECORD </h3>
...
<div id="idlogin" class="container">
   <form id="idlogform" class="form-horizontal" name="nmlogform" action="ip_editviewpat.php" method="post"  role="form">
