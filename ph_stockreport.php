...
//  $.ajaxSetup({async: false});
 ...
 //        alert("done ")
      ...
      /*if (!fields.succfail) { 
        ...
        alert(msg)
         return;}  09/042025 */  
        var aclsgstk=fields.aclsgstk;var adrugid=fields.adrugid;var adrugname=fields.adrugname;var aunitprice=fields.aunitprice;
  
            for(var i = 0; i < (drugid.length); i++) {
              rptaunitprice=0.00;rptaclsgstk=0;
           //   alert(adrugid[i]);alert(drugid[i])
               j = adrugname.indexOf(drugname[i])
         //      alert(j)
            if (j != -1) { 
               rptaunitprice=(aunitprice[j]*1).toFixed(2);rptaclsgstk=aclsgstk[j];}  
             var vcost=(rptaunitprice*rptaclsgstk).toFixed(2) 
              
                                               //addedby shoba   
           tablereport.row.add([subgroup[i],genericname[i],drugname[i],drugstrength[i],drugtype[i],rptaclsgstk,rptaunitprice,vcost])    
               .draw(false);  
	...
	.fail(function(xhr,status, error) {
			var temp="Request ph_getstockdata failed kindly contact programmmer status is "+xhr.status
			  alert(temp)
          $(document).off()
         return;
    })          		
