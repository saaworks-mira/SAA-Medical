...
set_error_handler("customError");	 
  $alldrugnm=array(); $alldrugid=array(); $alldrugtype=array(); $opstk=array(); $lastpricepur=array(); $allgenericname=array();$allsubgroup=array();
...
// Check connection
     ...
     $sql = "SELECT drugid, drugname, strength, type, stock, lastpricepur,subgroup,genericname FROM drug1 where enable = 'E' ORDER BY drugname";   // get all patient
 ...
 // july19   if (!$conn->query($sql)) 
   ...
   if ($result->num_rows > 0) 
     
    {
      ...
      $allgenericname[]=$row["genericname"]; $allsubgroup[]=$row["subgroup"];   
   ...
   {$errdesc = "Error: " . $sql . " " . $conn->error; $succfail=false; $operation="Select Drugs Empty Table"; 
    ...
    $succfail=true;
          $fields = array( "succfail" => $succfail,  
           ...
           "lastpricepur" => $lastpricepur, "stock" => $opstk,"genericname"=>$allgenericname,"subgroup"=>$allsubgroup);
