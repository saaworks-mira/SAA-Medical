...
//    if (countlast !=0){arraylength++};
      ...
      drugcount=0;drugs900 = drugid.slice(drugcount, drugcount+900);gstartdt=vstartdt;done900sets=false;
      getnext();
 }
...
function getnext(){ 
  var vpost=$.post("ph_opdgetstockdata.php", {palldrugs:"y", startdt:gstartdt, pdrugsall:drugs900})
           ...
           .done(function(result, status){  
            ...
            if (!fields.succfail) { 
               msg='Program terminating with error '+ fields.msg   
               alert(msg);done900sets=true;
               return;} 
            var aclsgstk=fields.aclsgstk;var aunitprice=fields.aunitprice;
            var vcost=0;
            for(var i = 0; i < (drugs900.length); i++) {
                 vcost=(aunitprice[i]*aclsgstk[i]).toFixed(2); 
                                                          //addedby shoba   
           tablereport.row.add( [drugname[drugcount+i],drugstrength[drugcount+i],drugtype[drugcount+i],aclsgstk[i],aunitprice[i],vcost])    
               .draw(false);   }
             if (done900sets){
             tablereport.draw(); 
         ...
         drugcount=drugcount+900; 
       if ( (drugcount+900) < drugid.length) {drugs900 = drugid.slice(drugcount, drugcount+900)} else {drugs900 = drugid.slice(drugcount);done900sets=true}; 
           getnext();return;
       }) 
       .fail(function(result, status){  
               msg='Program terminating with error '+ status   
               alert(msg);done900sets=true; $(document).off();
               return }) 
}                
