...
// create lock
$sql = "SELECT purchasedt , invoiceno, supplierid , suppliername , drugid, drugname,strength,type, batchno, expdt, opnstk,
 ...
 $prowcoldata[0]=$conn->real_escape_string($prowcoldata[0]); 
   ...
   drugname= '$prowcoldata[1]',strength= '$prowcoldata[2]',type= '$prowcoldata[3]', batchno= '$prowcoldata[4]', expdt= '$prowcoldata[5]',
...
//$plastpricepur[$i]=round(($plastpriceperpk[$i]/$temp[6]),2);
 $plastpricepur = round(($prowcoldata[13]/$prowcoldata[7]),2);
...
$sql = "SELECT lastpricepur, stock,stockvalue FROM drug1 
 ...
 //  $plastpricepur[$i]=round(($plastpriceperpk[$i]/$temp[6]),2);
   ...
   $sql = "update drug1 set plastpricepur ='$plastpricepur', stock = '$porigqty',stockvalue= stock*'$plastpricepur' where drugid='$porigdrugid'";
