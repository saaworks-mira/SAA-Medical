...
// Check connection
  ...
  $pinvno=$conn->real_escape_string($pinvno);
   ...
   $sql = "update purchase set invoiceno= '$pinvno' where purchaseid='$purchaseid[$i]'"; // feb 3 2018
   $sql = "update purchase set invoiceno= CONCAT(invoiceno,'/','$pinvno') where purchaseid='$purchaseid[$i]'";  // feb 3 2018
   $succfail=true ; 
       if (!$conn->query($sql))
