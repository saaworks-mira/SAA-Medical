...
#identerviewstartdt {margin-left: -29px;padding-left: -4px;padding-right: -4px;}
#identerviewenddt,#expdtview {margin-left: -28px;padding-left: -4px;padding-right: -4px;}
#viewinvno {margin-left: -50px;padding-left: -4px;padding-right: -4px;}	
#idviewalldrug {margin-left: -25px;}	
 /* 
...
<script type="text/javascript"> 
  ...
  var batchno=[]; var expdt=[]; var opngstk=[];var qtyperpack=[];var packs=[];var freepacks=[];var vopngstk=[]; var vclsgstk=[]; var vdruggn=[];
...
// 1 -drug 2-strength 3-type 4-batch 5-dt 6-stock 7-no/pk 8-packs 9-freepacks  10-qty  11-unitprice 12-Discounted/price 13-gross  14-tax% 15-taxamt  16-net 
 ...
 //        
  ...
  $('input[id^=idinp]').each(function(i, selected){       // get all input elements starting with idinp
    ...
    if (!document.getElementById(thisid).readOnly) {   // check only enterable fields
      if (this.type == "number") {                      // check if element is number type
       if (last2 == "09" || last2 == "15") {   //  check non mandatory numeric fields--  free packs,  tax
         if((document.getElementById(thisid).value =="")) 
...
var desc='<thead id="idthead"><tr><th>Delete</th><th>Drug name</th><th>Generic name</th><th>Strength</th><th>Type</th>'   //shoba
+'<th>Batch No</th>'+'<th>Exp Date</th><th>Opn Stock</th><th>Qty/Pack</th>'+
  ...
  var descreport='<thead id="idviewthead"><tr><th class="text-center">Drug name</th><th class="text-center">Generic Name</th><th class="text-center">Strength</th><th class="text-center">Type</th><th class="text-center">Purchasedt</th>'+
...
var temp0='<button type="button" class="btn btn-default btn-sm delete"  style="color:#FF0000;"><span class="glyphicon glyphicon-remove"></span></button>'
  ...
  var temp1=temp1a+temp1b 
   var temp18='<input type ="text" id="idinp-0118" class="cvvlong scclass flditem form-control" readonly><span class="centerstrong text-center" id="idinp-0118err"></span>'
 ...
 // append first row 
  var desc='<tr id="idrow01" class="tblrow">'+'<td id="idcol0100">' +temp0+ '</td>'+ '<td id="idcol0101">' +
           temp1+'</td>'+ '<td id="idcol0118">' + temp18+ '</td>'+ '<td id="idcol0102">' +temp2+ '</td>'+ '<td  id="idcol0103">'+temp3+'</td>'+
...
function getpurchasedata(vsup,vdrug,vstartdt,vendt,vexpdt,viewinvno)  {
  $.post("ph_getpurchdata.php", {
            psup:vsup,pdrug:vdrug,pstartdt:vstartdt,penddt:vendt,pexpdt:vexpdt,pinvno:viewinvno},
        function(result, status){ 
 //    alert(result)
        if (status) {
...
// below variables are retunbed from backend and contains all the drugs         
       ...
       vdrugname=fields.drugname;vdruggn=fields.genericname; vstrength=fields.strength; vdrugtype=fields.drugtype;
     ...
     netamt=fields.netamt; issueauthority=fields.issueauthority;   
      ...
      var purdtddmmyyyy=purchasedt[i].substr(6,2)+'/'+purchasedt[i].substr(4,2)+'/'+purchasedt[i].substr(0,4)
...
// now add to the report table  
        //    alert(vdruggn[i])
             ...
             tablereport.row.add( [
    vdrugname[i],vdruggn[i],vstrength[i],vdrugtype[i],purdtddmmyyyy,pursuppliername[i],invoiceno[i],batchno[i],vexpdt,vopngstk[i],qtyperpack[i],
...
// below variables are retunbed from backend and contains all the drugs         
    var  pqtyperpack=fields.qtyperpack;
		var  ptaxpercent=fields.taxpercentage;			// aug20                          shoba 23
		var qtyid="idinp-"+ thisrowid.slice(-2)+"07" 
		var taxpctid="idinp-"+ thisrowid.slice(-2)+"15" // aug20                    shoba 23
		document.getElementById(taxpctid).value=ptaxpercent // aug20
...
//			alert(idtype)
       document.getElementById(stockid).value=opstk[drugindx]
 //  alert(document.getElementById(stockid).value)                                               150718
...
// below variables are retunbed from backend and contains all the drugs         
    var  pqtyperpack=fields.qtyperpack;
					var  ptaxpercent=fields.taxpercentage;			// aug20 
		var taxpctid="idinp-"+ rowid.slice(-2)+"15" // aug20
		document.getElementById(taxpctid).value=ptaxpercent // aug20
...
//	if(e.which != 40) {return}  commented vikas june 6 and replaced by line below below
  ...
  var thisselid=thisid.substr(0,2)+thisid.substr(6) 
    ...
    if ( (e.which == 13) && ($("#"+thisselid).is(":visible")) ) 
         ...
         var idpriceerr='idinp-'+rowid+'12'+'err'
         var idgenericname= 'idinp-'+rowid+'18'
          var idgenericnameerr= 'idinp-'+rowid+'18'+'err'
         $('.info').attr('data-original-title', ""); 
...
//     document.getElementById(idprice).value=lastpriceperpk[drugindx];
			  document.getElementById(idprice).value=nondiscprice[drugindx];
//            alert(document.getElementById(idprice).value)                                    150718
     document.getElementById(idpriceerr).innerHTML="" 
...
// this means its all good and we proceed to add new row and set the id for all elements in the new row. 
  var keyCode = e.keyCode || e.which; 
	if ((keyCode == 13)  && ((this.id).slice(-2) == '01')) {return;}  // added by vikas june 6. If enter key on dropdown then dont validate row  // shoba added 18 for 18 columnsafter including genericname
...
//     var temp=i.toString();                     june 26 vikas
//     if (i < 10) {var colno= '0'+temp}          june 26 vikas
//     else {var colno=temp};                     june 26 vikas
//     var cellid = "idcol"+ rowno+ colno;        june 26 vikas
     var thiscell=thisrow.cells[i]
//     thiscell.setAttribute("id", cellid);  // commented june 26 vikas
      var colno=$(thiscell).attr('id').slice(-2)   //june 26 vikas
    var cellid = "idcol"+ rowno+ colno;  // june 26 vikas
     thiscell.setAttribute("id", cellid); // june 26 vikas
 ...
 //1 -drug 2-strength 3-type 4-batch 5-dt 6-stock 7-no/pk 8-packs 9-freepacks  10-qty 11-clos stock 12-unitprice 13-Discounted/price 13-gross  14-tax% 15-taxamt  16-net      
  // check for non mandatory fields which are numeric. they will be 0 if no entry. but they cannot be negative
        ...
        //      var tempexp= 'input[id^=idinp-'+getrowno+']'
            ...
            if (last2 == "09" || last2 == "15" ) {  // tax and freepacks  and discprice are non mandatory numeric fields which can be therefore 0
              if((document.getElementById(thisid).value =="") || 
...
$(document).on('click','#idpurbtnsrch',  function(){
   ...
   var vstartdt=$('#idviewinpstartdt').val();var venddt=$('#idviewinpenddt').val();var vexpdt=$('#idviewinpexpdt').val();
     ...
     if (sel !="Z") {
      ...
      var vinvnoview=document.getElementById("idviewinvno").value
			document.getElementById("idviewinvnoerr").innerHTML = "";
	   vinvnoview=vinvnoview.trim()
	   if (vinvnoview !== "") {
      var checkinput = /^[0-9a-zA-Z\/-]+$/; 
    if (!checkinput.test(vinvnoview) && vinvnoview.length > 0)  { // if input has non-alphabet characters
  	   document.getElementById("idviewinvnoerr").innerHTML="invalid input";$('#idpurbtnsrch').prop("disabled",false);return;
      }
		 }
     tablereport.clear();
     
//   $('#idtableview tr').slice(1).remove(); 
    getpurchasedata(vsup,vdrug,vstartdt,venddt,vexpdt,vinvnoview)
  
})

$(document).on('blur', '#idviewinvno', function(e){ 
	...
	var vinvnoview=document.getElementById("idviewinvno").value
      ...
      var checkinput = /^[0-9a-zA-Z\/-]+$/; 
    if (!checkinput.test(vinvnoview) && vinvnoview.length > 0)  { // if input has non-alphabet characters
  	   document.getElementById("idviewinvnoerr").innerHTML="invalid input";
      }
		 }	     									 
 }) 
    ...
    <div id="idviewalldrug" class="col-sm-3"> 
     <label>DRUG NAME:</label>
      ...
      <select select size="5" id="idviewdrug"  class=" cvlong form-control">   
			 ...
			 <div id="viewinvno" class= "col-sm-1 col-md-1">
        <label>INV NO:</label>
           <input type ="text"  class="form-control" id="idviewinvno"  value="" >
              <span class="centerstrong" id="idviewinvnoerr"></span>
      </div>
     ...
     <input id="idviewinpenddt" onkeydown="return false" type="text" maxlength="10" class="form-control fldview" name ="nmenddt">
      ...
      <div id="expdtview" class="col-sm-1 col-md-1">
         <label class="control-label">Exp Date</label>
        <input id="idviewinpexpdt" type="text" maxlength="5" class="cshort fldview form-control" placeholder="mm/yy">
