...
<script type="text/javascript"> 
  var drugname=[]; var drugid=[];var drugstrength=[]; var drugtype=[]; var opstk=[]; var lastpricepur=[];
...
//  $.ajaxSetup({async: false});
  ...
  if ( (vstartdt==datestr)  && ( vdrug=="Z") ) {// if for all drugs then get all stock  from drug master
      ...
      for(var i = 0; i < (drugname.length); i++) {
        ...
        tablereport.row.add( [drugname[i], drugstrength[i], drugtype[i], opstk[i],lastpricepur[i],vcost]) 
				  $('#idproc').removeClass('hidden') 
	         }
      ...
      if (pos != -1) { //search successful
         ...
         var  opt = drugname[i];  
           ...
           el.value = drugid[i].toString()+'--name'+drugname[i]+'--strength'+drugstrength[i]+'--type'+
						 drugtype[i]+'--stock'+opstk[i]+'--price'+lastpricepur[i];  
  ...
  <div  class="container-fluid">
   ...
   <div class="row form-group">
        <div class="col-sm-offset-2 col-sm-2">
          <label class="radio-inline text-success font16 "><strong>STOCK POSITION</strong></label></div>
