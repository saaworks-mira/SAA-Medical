...
//   var targetEl = document.getElementById(insertid); // handles spaces fine added vikas 01/05/2026

 //  $(targetEl).load(filen, function(response, status, xhr) {
 //  $( '#'+insertid ).load(filen, function(response,status,xhr){  // commented 01/05/2026
    var safeid = insertid.replaceAll(' ', '\\ ');
      ...
      //    var temp=$(targetEl).find('#idfile').html() // 01/05 vikas
          var temp = $('#' + safeid).find('#idfile').html();
          $("#idfilem").html(temp) 
	document.getElementById("idfilem").readOnly = true;
  //         $( '#'+insertid ).hide() // 01/05/2026 vikas
  //        $(targetEl).hide();  // 01/05/2026  vikas  
              $( '#'+safeid ).hide() // 01/05/2026 vikas    
        $( '#idpatmsgrow' ).addClass("hidden")
          $( '#idpatconfrow' ).addClass("hidden")
          $( '#idpateditrow' ).removeClass("hidden")
//					CKEDITOR.instances['idfilem'].setReadOnly(true);
//					CKEDITOR.instances['idfilem'].destroy();
//					 CKEDITOR.instances['idfilem'].config.readOnly = true;
        } 
     })  // end load
...
$(document).on('click','#idpatbtnconf', function (e) {	
 ...
 //       $("#"+insertid).find('#idfile').html(content) // commented 01/05/2026
     var targetEl = document.getElementById(insertid); // handles spaces fine added vikas 01/05/2026
     $(targetEl).find('#idfile').html(content) // 
    ...
    tempvar = tempvar.replaceAll(',', ''); // added vikas 01/05/2026
      tempvar = tempvar.replaceAll('(', ''); // added vikas 01/05/2026
     tempvar = tempvar.replaceAll(')', ''); // added vikas 01/05/2026        var reportname=tempvar+'.html';     // get report name
