...
// Check connection
  ...
  $sql = "SELECT  opdtestid,patientid, testid, opdid, opddate, ptname,testname,sex,age FROM opdtest where completion = 'N' 
          ORDER by ptname ASC, opddate DESC, testname ASC";  
...
// get all the fields and return to main program
    ...
    {  
      ...
      {$patientid[]=$row["patientid"];$ptname[]=$row["ptname"];$testid[]=$row["testid"];$testname[]=$row["testname"];
       $opdid[]=$row["opdid"];$opddate[]=$row["opddate"]; $opdtestid[]=$row["opdtestid"];$sex[]=$row["sex"]; $age[]=$row["age"];    
     $alldata[]=$row["patientid"]."--".$row["testid"]."--".$row["opddate"]."--".$row["ptname"]."--".
       $row["testname"]."--".$row["opdid"]."--".$row["opdtestid"]."--".$row["age"]."--".$row["sex"];
              }  // get names and id of all patients  patid--testid--date--name--testname--opdid--opdtestid--age--sex
    
            $succfail=true;
          $fields = array( "succfail" => $succfail, "opdid" => $opdid,"opdtestid" => $opdtestid, "opddate" => $opddate, "testsfound" => true,
          "testid" => $testid, "testname" => $testname, "sex" => $sex, "age" => $age, "ptname" => $ptname,
