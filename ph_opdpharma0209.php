...
// june 9 vikas   added genericname below         
          ...
          pdrugid[i].toString() + '--name'+ pdrugname[i] + '--strength'+ pdrugstrength[i] +'--type' + pdrugtype[i] +'--generic' + pdruggenericname[i] + '" disabled="disabled">'+pdrugname[i]+'----'+'(QTY)  '+pqtyissued[i]+'----(ISSUED)  '+
              pdtofissue[i].substr(6,2)+'/'+pdtofissue[i].substr(4,2)+'/'+pdtofissue[i].substr(0,4)+'</li>';     
          }
...
$(document).on('keyup click','input[id^=idtextsel],input[id^=idtextviewdrug]',  function(e) {
 ...
 if(keyCode == 13) {return}}
  ...
  var thisid=this.id;
/*	
 if (thisid.indexOf("idtextsel") != -1) {   
    $('input[id^=idtextsel]').each(function(i, selected){     // get all rows
     var thiseachid=this.id;
     if (thisid != thiseachid) {        // close all open drug dropdowns of other rows
     var thisselid=thiseachid.substr(0,2)+thiseachid.substr(6) 
      
       var thistextid=thiseachid;
       var thistextiderr=thistextid+'err' 
      if (  $("#"+thisselid).is(":visible"))   // if its open
        {         
        var textinp = (document.getElementById(thistextid).value).trim()      
    ...
    if (checkposinarray(drugname, textinp) == -1) {
       ...
       document.getElementById(thistextiderr).innerHTML = "";
       var idopdstk="idinp-"+ thiseachid.substr(10,2)+"05"
       var drugindx = pos; 
...
$('[id$=err]').each(function(i, selected)
    ...
    $( "p span" ).first().addClass( "highlight" );"input[type='checkbox']"
*/    
