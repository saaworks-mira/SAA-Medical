...
$pstartyyyy=substr($pstartdt,6,4);$pstartmm=substr($pstartdt,3,2);$pstartdd=substr($pstartdt,0,2);
  ...
  $sql = "SELECT  opdtestid,patientid, testid, opdid, opddate, ptname,testname,sex,age FROM opdtest where completion = 'Y'
          AND patientid ='$ppat' AND opddate >= '$pstartdt' AND opddate <= '$penddt'
    ...
    {  
     ...
     $alldata[]=$row["patientid"]."--".$row["testid"]."--".$row["opddate"]."--".$row["ptname"]."--".
       $row["testname"]."--".$row["opdid"]."--".$row["opdtestid"]."--".$row["age"]."--".$row["sex"];
          ...
          "testid" => $testid, "testname" => $testname, "sex" => $sex, "age" => $age, "ptname" => $ptname, "patientid" => $patientid, "alldata" => $alldata);
