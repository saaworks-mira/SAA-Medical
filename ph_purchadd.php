...
$bal=array_fill(0,count($prowcoldata),0);$storedrugs=array_fill(0,count($prowcoldata),0); //13/4 vikas fill all the balances 

 for ($i = 0; $i < count($prowcoldata); $i++) {
       $drug = explode("--", $prowcoldata[$i][0]);
       $drugid=$drug[0];
       $drugnm=substr($drug[1],4);
       $druggn=substr($drug[4],11);
...
//       $drugtype=$conn->real_escape_string(substr($drug[3],4));
   ...
   //from  here   13/4 vikas
   if (($indx = array_search($drugid, $storedrugs)) !== FALSE) {     
      $temp[10]=$bal[$indx]+$temp[9];  
      $temp[5]=$bal[$indx];
   } 
   else {
   $storedrugs[$i]=$drugid; 
...
// till here 13/4 vikas   
   $sql = "INSERT INTO purchase (purchasedt,invoiceno, supplierid, suppliername, drugid, drugname,genericname,strength,type, batchno, expdt, opnstk,qtyperpack,
   packs, freepacks, totalqty, clsstk,  unitprice, discunitprice, grossprice, taxpercent, tax, netamt, remarks, issueauthority, timefixed)   
   VALUES ('$ppurchdt', '$pinvno', '$psupplierid', '$psuppliername','$drugid','$drugnm','$druggn', '$temp[1]','$temp[2]', '$temp[3]',  '$temp[4]', '$temp[5]',  '$temp[6]',
 ...
 $bal[$indx]=$temp[10];  // put the new balance vikas 10/4/2018 
